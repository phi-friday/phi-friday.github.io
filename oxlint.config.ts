/**
 * NOTE: JavaScript/TypeScript config files are experimental,
 * So we write the config to .oxlintrc.json for now, until .mjs/.cjs support is stable.
 */
...
const banner = `/**
 * NOTE: This file is auto-generated by oxlint.config.ts
 * Do not edit this file directly.
 * Instead, edit oxlint.config.ts and run the lint:oxlint:generate script.
 */
`;
const rc = JSON.stringify(config, null, 2);
const temp_filename = `.oxlintrc-${Date.now()}.json`;
const temp_path = path.join(tmpdir(), temp_filename);
process.on("exit", () => {
  if (fs.existsSync(temp_path)) {
    fs.unlinkSync(temp_path);
  }
});
fs.writeFileSync(temp_path, banner + rc);
fs.renameSync(temp_path, path.join(import.meta.dirname, ".oxlintrc.json"));
